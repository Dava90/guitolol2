...
const UserList = ({ handleLogout }) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await fetch("https://apitolol.vercel.app/api/users", {
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://apitolol.vercel.app/api/users/${selectedUser .id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteUser  = async (userId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://apitolol.vercel.app/api/users/${userId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
