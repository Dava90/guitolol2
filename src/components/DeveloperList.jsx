...
const DeveloperList = ({ handleLogout }) => {
  ...
  const fetchDevelopers = async () => {
    ...
    try {
      const response = await fetch("https://apitolol.vercel.app/api/developers", {
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://apitolol.vercel.app/api/developers/${selectedDeveloper.id_dev}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteDeveloper = async (developerId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://apitolol.vercel.app/api/developers/${developerId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
