...
const PublisherList = ({ handleLogout }) => {
  ...
  const fetchPublishers = async () => {
    ...
    try {
      const response = await fetch("https://apitolol.vercel.app/publishers", {
        headers: {
  ...
  const handleAddPublisher = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch("hhttps://apitolol.vercel.app/publishers", {
        method: "POST",
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://apitolol.vercel.app/publishers/${selectedPublisher.id_pub}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeletePublisher = async (publisherId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://apitolol.vercel.app/publishers/${publisherId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
