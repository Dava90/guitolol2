...
const GenreList = ({ handleLogout }) => {
  ...
  const fetchGenres = async () => {
    ...
    try {
      const response = await fetch("https://apitolol.vercel.app/genres", {
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://apitolol.vercel.app/genres/${selectedGenre.id_genre}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteGenre = async (genreId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://apitolol.vercel.app/genres/${genreId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
