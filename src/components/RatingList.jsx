...
const RatingList = ({ handleLogout }) => {
  ...
  const fetchRatings = async () => {
    ...
    try {
      const response = await fetch("https://apitolol.vercel.app/ratings", {
        headers: {
  ...
  const handleSave = async (event) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`https://apitolol.vercel.app/ratings/${selectedRating.id_rate}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteRating = async (ratingId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://apitolol.vercel.app/ratings/${ratingId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
